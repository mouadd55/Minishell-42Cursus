...
void open_heredoc(t_list *list, t_command **final_list, t_env **envr)
{
	...
	save = ft_strdup(v.val);
	v.tmp_value = ft_strdup(".");
	v.val = ft_strjoin(v.tmp_value, v.val);
	if (open(v.val, O_RDONLY) != -1)
		v.val = ft_strjoin(v.val, ft_itoa(rand()));
	v.fd = open(v.val, O_CREAT | O_RDWR | O_APPEND, 0777);
	while (1)
	{
		v.str = readline("Heredoc> ");
		if (!v.str || !ft_strcmp(v.str, save))
		{
			if (!v.str)
				ft_printf_fd("Minishell: warning: here-document delimited by end-of-file (wanted '%s')\n", 2, save);
			close(v.fd);
			unlink(v.val);
			free(v.str);
			free(v.val);
